...
import static com.protasknubyyynx.web.rest.TestUtil.createUpdateProxyForBean;
...
import com.protasknubyyynx.service.mapper.StatusMapper;
...
class StatusResourceIT {
    ...
    void createStatus() throws Exception {
        ...
        var returnedStatusDTO = om.readValue(
            restStatusMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(statusDTO)))
    ...
    void putExistingStatus() throws Exception {
        ...
        restStatusMockMvc
            .perform(
                put(ENTITY_API_URL_ID, statusDTO.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(statusDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamStatus() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restStatusMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(statusDTO)))
    ...
    void patchWithMissingIdPathParamStatus() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restStatusMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(statusDTO)))
            .andExpect(status().isMethodNotAllowed());
